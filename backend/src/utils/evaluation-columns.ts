...
export interface EvaluationColumnDefinition extends Record<string, unknown> {
  key: string;
  label: string;
...
function mergeLockedColumn(
  fallback: EvaluationColumnDefinition,
  existing: Partial<EvaluationColumnDefinition> | undefined,
...
export function normalizeEvaluationColumnSchema(
  ...
  const required = REQUIRED_COLUMNS.map((column) => mergeLockedColumn(column, byKey.get(column.key)));
  ...
  if (!hasSystemColumns) {
    ...
    return [
      ...required,
      ...SYSTEM_EVALUATION_COLUMNS.slice(0, 3).map((column) => mergeLockedColumn(column, undefined)),
      ...templateColumns,
      ...SYSTEM_EVALUATION_COLUMNS.slice(3).map((column) => mergeLockedColumn(column, undefined)),
    ];
  }
  ...
  const ordered = unique
    .map((column) => {
      ...
      if (requiredColumn) return mergeLockedColumn(requiredColumn, column);
      const systemColumn = SYSTEM_EVALUATION_COLUMNS.find((definition) => definition.key === column.key);
      return systemColumn ? mergeLockedColumn(systemColumn, column) : normalizeTemplateColumn(column);
    });
  ...
  const missingRequiredColumns = REQUIRED_COLUMNS
    ...
    .map((column) => mergeLockedColumn(column, undefined));
  for (const column of SYSTEM_EVALUATION_COLUMNS.filter((item) => !orderedKeys.has(item.key))) {
    const normalized = mergeLockedColumn(column, undefined);
