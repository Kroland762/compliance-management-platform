...
企业合规审计管理平台，基于 B/S 架构，涵盖**合规评估**、**产品合规**与**账户审计**，支持 schema 级租户隔离、RBAC 权限体系、版本化合规档案、多类型只读数据库数据源和规则引擎自动化审计。
...
- 风险是独立主对象：既可来自合规评估/不符合项，也可由具备 `risks:create` 的角色从日常运维、内部审计、安全事件等来源人工创建；人工风险不属于评估项目，并强制指定独立审核人
- 人工风险与评估风险共用确认、接受、整改、验证和关闭生命周期；独立风险不会进入或阻塞任何评估项目的关闭门禁
...
- 风险台账支持创建方式、发现来源、项目、状态、等级、策略、部门、本人相关、日期及逾期筛选；导出保留人工来源说明与引用
...
### 账户审计
- PostgreSQL、MySQL、SQL Server、Oracle、SQLite 只读数据源与 CSV 内存导入；字段映射按数据源（系统）保存，数据库同步和同一系统重新上传 CSV 时自动复用
- CSV 导入先预览并校验表头、账户 ID 和字段漂移，确认后事务切换批次；原始文件不落盘，仅保留哈希和导入元数据
...
- **数据源边界**：数据库直连拒绝任意 SQL 与危险标识符，按方言使用只读表和字段白名单，并默认阻断内网和云元数据地址；CSV 限制类型、大小与行数并仅在内存中解析
...
支持基线：Node.js `>=22.12 <23`、PostgreSQL 14+。生产数据只允许通过版本化迁移器变更。
...
macOS 也可以直接双击项目根目录的 `start-dev.command` 一键启动本机 PostgreSQL、后端和前端。脚本会复用已运行的健康服务、检查端口占用，并将日志写入 `.dev-logs/`；按 `Ctrl+C` 可关闭本次由脚本启动的前后端服务。
首次初始化仍需先执行 `npm run migrate:up` 和显式 seed；开发脚本只负责启动本机数据库与开发服务，不会代替版本化迁移或创建管理员。
...
独立风险由 `026_tenant_independent_risks` 启用。该迁移会将 `risk_records.taskId` 放宽为可空，
回填历史风险来源并把项目删除关系改为 `RESTRICT`。一旦产生独立风险，down migration 会拒绝执行；
运行时回退应先关闭前端创建入口并保留兼容数据库。对象关系和状态门禁见
[风险关系图说明](./风险关系图说明.md)。

迁移记录以 `(migration_id, schema_name)` 为主键并校验迁移内容；迁移器使用 PostgreSQL
advisory lock 防止并发执行。`migrate:down` 只允许对最后一个迁移进行显式确认回滚。
生产容器使用编译后的 `npm run migrate:prod` 和 `npm run seed:prod`；seed 始终显式执行。
不得以 `sync({ alter: true })` 代替版本化迁移。
关系图写接口使用 `If-Match` 乐观锁；评估认领使用行锁防止并发抢占；风险创建、整改提交和风险导出使用持久化
`Idempotency-Key`，以支持安全重试和并发防重。

单台云服务器、域名和 HTTPS 的完整操作见 [上云部署指南](./上云部署指南.md)；
数据库升级、存量迁移、备份恢复和故障排查见 [部署与升级指南](./部署与升级指南.md)。

生产 Compose 会将 PostgreSQL、后端和前端端口限制在 `127.0.0.1`，由主机 Nginx 统一暴露
80/443。复制 `.env.docker.example` 并填写正式域名和独立密钥后，可先执行：

```bash
./deploy/cloud-preflight.sh .env
```

预检不会启动容器或修改数据库；生产启动仍必须按“备份/配置 → 构建 → 版本化迁移 →
显式 seed（仅全新安装）→ 启动 → readiness → HTTPS 业务验收”的顺序执行。
...
│   │   └── utils/          # 前端工具函数
│   └── package.json
├── deploy/                 # 云部署预检与主机 Nginx HTTPS 模板
├── docker-compose.yml      # 单机生产编排，业务端口仅绑定回环地址
└── README.md
```
