...
- 不符合项验证、直接整改与升级风险按各自动作的数据范围授权，保留禁止自审规则。
- 风险详情提供独立审核人分配入口；读取与分配权限足够，资料编辑仍单独要求修改权限。
- 数据源字段预览保留自定义 schema 和 TLS；评估向导的审计员保存失败可在同一草稿重试。
- 新增 027 补偿迁移，仅修复可确认由 012 生成且未被编辑、所有关联风险已终态的不符合项；保存回退快照，不改原风险结论和历史迁移校验。
- Compose 沿用默认项目名；既有部署必须保持原项目名与数据卷映射，详见升级指南。
- 更新 Joi、Multer、body-parser，并固定安全版本 qs，修复生产依赖审计问题。

## 2026-09-11 — 风险流程与旧数据导入修复

- 替换评估风险来源时同步不符合项关系，移除最后关联后恢复待处置；确认门禁验证两类关系一致，避免接受或关闭错误的不符合项。
- 评估风险编辑保留只读来源；人工风险资料与审核人分开保存，成功后同步版本号，分配失败可修正后重试。
- 审核人资格使用成员多角色权限并集，要求可读取、确认并验证；整改验证按钮与 API 权限保持一致。
- 已逾期筛选与选定状态取交集，不再覆盖状态条件。
- 来源关系集合、关系类型或说明变更均递增受影响不符合项版本；整改窗口的风险候选按标题检索及排序，修复查询不存在的名称字段导致的错误。
- 评估向导切换步骤后保留所选模板，发布预览显示正确的评估行数。
- 成员迁移清单兼容 004 阶段及 026 前后的字段；旧 public 风险导入补齐来源和创建人，保留事务、原数据及哈希核对。现有迁移内容与约束保持不变。

## 2026-08-31 — 独立风险与日常运维来源

- 风险升级为独立主对象：新增人工创建模式及日常运维、内部审计、安全事件、投诉反馈、监管变化、第三方等发现来源；评估风险和不符合项升级链路保持兼容。
- 新增 `026_tenant_independent_risks`，允许风险不关联评估项目，回填历史创建方式/来源/创建人，增加独立审核人和来源一致性约束，并使用受保护的 down 门禁。
- 新增风险创建/编辑页面、台账服务端分页筛选、来源条件详情、可读人员/部门信息，以及“待确认风险”和独立风险整改验证待办。
- 人工风险由独立审核人确认和验证；整改负责人仍不得自审。独立风险不参与评估项目关闭门禁。
- 风险/整改通知写入幂等业务事务；风险趋势按发现时间统计，Excel 导出增加创建方式、人工来源说明、引用和空项目字段。
- 回退时优先关闭独立风险创建入口并保留兼容数据库；存在独立风险后不得强制执行 026 down。
